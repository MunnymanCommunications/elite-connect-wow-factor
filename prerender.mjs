 ...
 *
 * What this does:
 *   1. Auto-discovers every <Route path="..."> in src/App.tsx via
 *      route-discovery.mjs. Wildcard, gated, and /admin/ routes are skipped.
 *   2. Builds the SSR bundle with Vite. src/entry-server.tsx is regenerated
 *      from App.tsx by scripts/generate-entry-server.mjs (run first by the
 *      build pipeline).
 ...
 *
 * No hand-maintained route list. App.tsx is the single source of truth.
 *
 * CLIENT SETUP:
 *   1. BASE_URL — set to the client's production domain
 *   2. INTERNAL_ROUTES — pages that should prerender but stay out of sitemap
 *      (test pages, internal partner pages, etc.). Usually empty.
 *   3. fallbackMetadata() — change the company name in the title/description
 *      template to the client's actual name
 */
...
// ⬇️ CLIENT SETUP: production canonical base URL
export const BASE_URL = process.env.PRERENDER_BASE_URL || "https://elitecardpro.com";

// ⬇️ CLIENT SETUP: routes that should prerender but NOT appear in sitemap.xml
...
/**
 * Fallback title/description for routes that have no SEO_METADATA entry.
 * The verifier flags these as warnings so a human can write real copy later.
 *
 * ⬇️ CLIENT SETUP: replace "Client Name" with the actual brand
 */
...
/**
 * Append any discovered, non-internal routes to public/sitemap.xml that aren't
 * already present. Existing entries (priority/changefreq) are preserved.
 */
...
async function prerender() {
  ...
  if (gated.length)
    console.log(`   Skipping ${gated.length} auth-gated route(s): ${gated.join(", ")}`);
  if (excluded.length)
    console.log(`   Skipping ${excluded.length} excluded route(s): ${excluded.join(", ")}`);
  ...
  // Strip any pre-existing canonical so this script is idempotent (safe to
  // re-run without doubling tags).
  const template = fs
